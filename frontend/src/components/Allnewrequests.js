...
export default class Allnewrequests extends Component{
  constructor(props){
    super(props);
...
onDelete =(id)=>{
  axios.delete(`/nrequest/delete/${id}`).then((res)=>{
    alert("Delete new Requests Sucessfully");
    this.retrieveNrequests();
  })
}
  ...
  <thead>
    ...
    { this.state.nrequests.map((nrequests,index)=>(
      <tr>
        ...
        <td>
          <a className ="btn btn-danger"  href="#" onClick={()=>this.onDelete(nrequests._id)} >Delete</a>&nbsp;  
      </td>
         </tr>
    ))}
